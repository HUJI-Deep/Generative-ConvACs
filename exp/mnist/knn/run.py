...
def test(name, training_data, dataset, K):
    if os.path.isfile(name):
        ...
        touch(name)
    cmd = [TEST_SCRIPT, '--data_dir', join(SCRIPT_DIR,MNIST_DATA), '--missing_data_dir', 
           join(SCRIPT_DIR,dataset), '--k', str(K), '--dataset', 'mnist']
    cmd = ' '.join(cmd)
    print cmd
...
try:
    for db in DBS:
        for mar_prob in MAR_PROBS:
            ...
            dataset = '%s_%s' % (MNIST_TEST, base)
            name = 'out_test_%s.csv' % (base)
            test(name, MNIST_DATA, dataset, K)
