...
def summary(table):
    ...
    accuracies = table.select(['hyper__mar_prob','test_results0_0_accuracy'])
    accuracies = accuracies.rename({'test_results0_0_accuracy': 'accuracy', 'hyper__mar_prob': 'N'})
    accuracies = accuracies.order_by(['N'])
